...
main(){
    ...
    cd RDGOAT > /dev/null 2>&1 || { mkdir RDGOAT && cd RDGOAT ; } ; curl -s -O https://raw.githubusercontent.com/RichardDeodutt/Deployment-5/main/Scripts/statuscheck.sh && chmod +x statuscheck.sh && curl -s -O https://raw.githubusercontent.com/RichardDeodutt/Deployment-5/main/Scripts/libstandard.sh && chmod +x libstandard.sh && ./statuscheck.sh || exiterror
}
