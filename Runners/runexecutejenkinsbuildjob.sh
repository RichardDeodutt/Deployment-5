...
#Richard Deodutt
#10/31/2022
#This script is meant to run the script to execute a Jenkins build job on ubuntu
...
main(){
    ...
    cd RDGOAT > /dev/null 2>&1 || { mkdir RDGOAT && cd RDGOAT ; } ; curl -s -O https://raw.githubusercontent.com/RichardDeodutt/Deployment-4/main/Scripts/executejenkinsbuildjob.sh && chmod +x executejenkinsbuildjob.sh && curl -s -O https://raw.githubusercontent.com/RichardDeodutt/Deployment-4/main/Scripts/libstandard.sh && chmod +x libstandard.sh && ./executejenkinsbuildjob.sh || exiterror
}
